...
def generate_corrupted_values(corruption_style, snps_to_remove, data, fold_indices):
    if corruption_style == 'missing':
        ...
        #vals_to_substitute = np.zeros_like(snps_to_remove)
        # generate values that have same distribution as before
        #unique_feats, counts = np.unique(snps_to_remove.flatten(), return_counts=True)
        #for feat, count in zip(unique_feats, counts):
        #    # randomly sample values of feature 
        #    #questions: 1) should we sample with replacement? 2) should we use the full dataset?
        #    vals_to_substitute[snps_to_remove == feat] = np.random.choice(data[:,feat], 
        #                                                                  replace=False, 
        #                                                                  size=count)
        #return vals_to_substitute
            
        

    #def corrupt_values
    #    for i in range(len(attrs)):
    #        mask = (attrs[i] <= threshold[i, None, None]).flatten()
    #        test_set_corrupted.data[i].flatten()[mask] = median_values.flatten()[mask]
...
def main():
    ...
    if args.task == 'classification':
        ...
        task_handler = ClassificationHandler(args.dataset, 
                                             criterion)

    elif args.task == 'regression':
        criterion = nn.MSELoss()
        task_handler = RegressionHandler(args.dataset, 
                                         criterion)
    ...
    # Fold indices
    indices_byfold = np.load(args.partition, 
                             allow_pickle=True)
    fold_indices = indices_byfold['folds_indexes'][fold]
    ...
    for corruption_percent in corr_percent_list:
        ...
        if args.random_baseline:
            if args.attribution_score == 'none_raw':
                # keep sign info
                random_style = 'random_incl_baseline_and_sign'
            else:
                random_style = 'random_incl_baseline'

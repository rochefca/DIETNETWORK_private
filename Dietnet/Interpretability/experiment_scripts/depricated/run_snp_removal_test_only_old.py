...
def main():
    ...
    for percentile_to_remove in [100*(0.85**i) for i in range(0, 4)]: #range(0, 21)
        ...
        if args.baseline:
            ...
            to_keep[np.random.choice(to_keep.shape[0], amt_to_keep, replace=False)] = False

        formatted_genotypes, feature_scaling = utils.match_features2(du.FoldDataset.data_x, np.arange(len(to_keep))[to_keep], np.arange(len(to_keep)))
